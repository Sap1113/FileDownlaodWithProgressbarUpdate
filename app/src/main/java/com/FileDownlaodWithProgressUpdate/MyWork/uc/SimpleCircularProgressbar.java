package com.FileDownlaodWithProgressUpdate.MyWork.uc;
...
public class SimpleCircularProgressbar extends View {
	...
	public SimpleCircularProgressbar(final Context context,
		...
		mContext = context;
		final TypedArray attributes = mContext.obtainStyledAttributes(attrs,
				R.styleable.SimpleCircularProgressbar, defStyleAttr, 0);

		if (attributes != null) {
			try {
				mMaxProgress = attributes.getFloat(
						R.styleable.SimpleCircularProgressbar_maxProgress,
						100.0f);
				// mCurrentProgress = attributes.getFloat(
				// R.styleable.SimpleCircularProgressbar_progress, 0.0f);
				mStartAt = attributes.getInteger(
						R.styleable.SimpleCircularProgressbar_startAt, 1); // default
																			// will
																			// be
																			// top
				mRotation = attributes.getInteger(
						R.styleable.SimpleCircularProgressbar_rotation, 0);
				mProgressbarColor = attributes.getColor(
						R.styleable.SimpleCircularProgressbar_progressColor,
						Color.BLUE);
				mProgressbarBackgroundColor = attributes.getColor(
						R.styleable.SimpleCircularProgressbar_backgroundColor,
						Color.parseColor("#fea601"));
				mThickness = attributes
						.getInteger(
								R.styleable.SimpleCircularProgressbar_progressbar_thickness,
								4);
			} finally {
				attributes.recycle();
			}

			init(mProgressbarColor, mProgressbarBackgroundColor);
		}
	}

	// shreyans
	public void setCircleColor(int mProgressbarColor,
			int mProgressbarBackgroundColor) {
		// init(mProgressbarColor, mProgressbarBackgroundColor);
	 ...
	 */
	private void init(int mProgressbarColor, int mProgressbarBackgroundColor) {
		mProgressbarPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
		mProgressbarPaint.setColor(mProgressbarColor);
		mUnProgressbarPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
		mUnProgressbarPaint.setColor(mProgressbarBackgroundColor);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		int defH = getDefaultSize(getSuggestedMinimumHeight()
				+ getPaddingBottom() + getPaddingTop(), heightMeasureSpec);
		int defW = getDefaultSize(getSuggestedMinimumWidth() + getPaddingLeft()
		...
		if (defH < defW) {
			...
			mOuterCircleBounds.set(pHalfW - halfH, pHalfH - halfH, pHalfW
					+ halfH, pHalfH + halfH);
			mUnprogressedBounds.set(pHalfW - halfH, pHalfH - halfH, pHalfW
					+ halfH, pHalfH + halfH);
		} else {
			// width is small
			final float pHalfH = defH / 2f;
			...
			mOuterCircleBounds.set(pHalfW - halfW, pHalfH - halfW, pHalfW
					+ halfW, pHalfH + halfW);
			mUnprogressedBounds.set(pHalfW - halfW, pHalfH - halfW, pHalfW
					+ halfW, pHalfH + halfW);
		}
	...
	private void printClockwise(final Canvas canvas) {
		...
		case 0:
			// left
			canvas.drawArc(mOuterCircleBounds, 180, mTraverse, false,
					mProgressbarPaint);
			canvas.drawArc(mUnprogressedBounds, 180, -((360 - mTraverse)),
					false, mUnProgressbarPaint);
			break;

		case 1:
			// top
			canvas.drawArc(mOuterCircleBounds, 270, mTraverse, false,
					mProgressbarPaint);
			canvas.drawArc(mUnprogressedBounds, 270, -((360 - mTraverse)),
					false, mUnProgressbarPaint);
			break;

		case 2:
			// right
			canvas.drawArc(mOuterCircleBounds, 0, mTraverse, false,
					mProgressbarPaint);
			canvas.drawArc(mUnprogressedBounds, 0, -((360 - mTraverse)), false,
					mUnProgressbarPaint);
			break;

		case 3:
			// bottom
			canvas.drawArc(mOuterCircleBounds, 90, mTraverse, false,
					mProgressbarPaint);
			canvas.drawArc(mUnprogressedBounds, 90, -((360 - mTraverse)),
					false, mUnProgressbarPaint);
			break;
		}
	}
	...
	private void printAntiClockwise(final Canvas canvas) {
		...
		case 0:
			// left
			canvas.drawArc(mOuterCircleBounds, 180, 360, false,
					mProgressbarPaint);
			canvas.drawArc(mUnprogressedBounds, 180, (360 - mTraverse), false,
					mUnProgressbarPaint);
			break;

		case 1:
			// top
			canvas.drawArc(mOuterCircleBounds, 270, 360, false,
					mProgressbarPaint);
			canvas.drawArc(mUnprogressedBounds, 270, (360 - mTraverse), false,
					mUnProgressbarPaint);
			break;
		...
		case 3:
			// bottom
			canvas.drawArc(mOuterCircleBounds, 90, 360, false,
					mProgressbarPaint);
			canvas.drawArc(mUnprogressedBounds, 90, (360 - mTraverse), false,
					mUnProgressbarPaint);
			break;
		}
	}
	...
	/**
	 * Sets the passed progress to current progress and updates the Progressbar
	 * 
	 * @param progress
	 *            current value traced by progressbar
	 */
	...
	/**
	 * Sets the starting point of progress whether it is top, bottom , right or
	 * left
	 * 
	 * @param from
	 *            position from which it progress should start
	 */
	...
	/**
	 * Sets color which will be highlighted. It will be actual progress color
	 * 
	 * @param color
	 *            color which will be color which will be set to progressed bar
	 */
	...
	/**
	 * Sets color which won't be highlighted. It will be actual un-progressed
	 * color
	 * 
	 * @param color
	 *            color which will be set to un-progressed bar
	 */
